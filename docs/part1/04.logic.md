...
ES5 Array의 고차함수들이 반환값으로 새로운 Array 객체를 반환하여 각각에 영향을 미치지 않도록 하는 것과 같이 RxJS의 operator는 항상 새로운 Observable을 반환함으로써 Array의 고차함수와 같이 `Immutable 객체`를 반환한다. 반환된 Observable은 `Read Only`이기 때문에 객체의 내용도 변경하지도 못한다.
