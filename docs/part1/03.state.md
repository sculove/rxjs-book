...
이와 관련된 내용은 3장에서 다룰 `로직 오류 대한 문제`에서 다시 한번 다루도록 하겠다.
...
## RxJS는 리액티브 하다.
RxJS는 Observable에서 데이터가 발생하게되면 Observer에게 자동으로 그리고 빠르게 변경된 데이터를 전달한다.
이를 보고 `RxJS는 리액티브하다`고 이야기할수 있다.
...
개선된 Observer 패턴을 적용한 RxJS는 궁극적으로 `리액티브 프로그래밍(Reactive Programming)`을 사용할 수 있게 도와준다.
